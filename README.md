...
| Флаг | Описание |
|------|----------|
| `-m, --model` | Модель (по умолчанию: `google/gemini-3.1-flash-lite-preview`) |
...
| Флаг | Описание |
|------|----------|
| `-m, --model` | Модель (по умолчанию: `google/gemini-3.1-flash-image-preview`) |
...
| Флаг | Описание |
|------|----------|
| `-m, --model` | Модель (по умолчанию: `google/veo3_fast`) |
| `-o, --output` | Путь для сохранения файла |
...
Файл [`SKILL.md`](SKILL.md) описывает возможности CLI в формате, понятном AI-агентам (Claude Code, Cursor, Cline и др.). Агент может читать этот файл и использовать `ai` как инструмент для генерации контента.
